  ...
  static async getDashboardSummary() {
    ...
    const [
      ...
      db.from("orders")
        ...
        .neq("stage", "cancelled"),
      db.from("v_admin_products").select("*").eq("sold_out", true).order("updated_at", { ascending: false }).limit(12),
    ...
    const revenue = revenueData.reduce((sum: number, row: { total?: number; total_amount?: number }) => sum + (row.total ?? row.total_amount ?? 0), 0);
