...
import { cancelWaybill, isBlueDartConfigured } from "./bluedart.service.js";
    ...
    if (awb && isBlueDartConfigured()) {
      ...
      if (!cancelResult.cancelled) {
        console.warn(
          ...
          "Proceeding with order cancellation — manual Blue Dart intervention may be needed."
        );
      }
    }
  }
...
export async function processRtoRestockAdmin(params: {
  ...
  adminId: string;
  note?: string;
}): Promise<{ stockRestored: boolean; refundInitiated: boolean; message: string }> {
  ...
  // 4. Update shipment status to RTO if active shipment exists
  await db
    .from("shipments")
  ...
  return {
    stockRestored,
    refundInitiated,
    message: `RTO processing complete. ${stockRestored ? "Stock returned to inventory." : "Stock was already restored."} ${refundInitiated ? "Refund initiated via Razorpay." : order.payment_method === "cod" ? "COD order — no refund required." : "Refund pending admin retry."}`,
  };
}
